...
export default function UserOnboarding() {
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      ...
      case 5:
        ...
        if (!f.password.trim() || f.password.length < 6) newErrors.password = 'Min 6 characters';
  ...
  // ---------- Styles (sleek card, progress, blue/white) ----------
  const styles = {
    container: {
    ...
    header: {
      padding: '20px 24px',
      background: 'linear-gradient(135deg, #0a66c2 0%, #0a66c2 60%, #004182 100%)',
      color: '#fff'
    },
    ...
    dot: (active) => ({
      ...
      background: active ? '#0a66c2' : '#d1d5db'
    }),
    grid: {
      display: 'grid',
    ...
    btnPrimary: {
      ...
      color: '#fff',
      boxShadow: '0 10px 22px rgba(10,102,194,.25)'
    },
    btnGhost: {
      background: '#eef2f6',
      color: '#1f2937'
        ...
        <div style={styles.card}>
          ...
          <form onSubmit={handleSubmit} style={styles.body}>
            ...
            {currentStep === 1 && (
              <>
                <div style={styles.grid}>
                  <Field styles={styles} label="First Name" name="firstName"
                    value={formData.firstName} onChange={handleInputChange}
                    placeholder="e.g. Aisha" autoComplete="given-name" />
                  <Field styles={styles} label="Middle Name" name="middleName" required={false}
                    value={formData.middleName} onChange={handleInputChange}
                    placeholder="(optional)" autoComplete="additional-name" />
                  <Field styles={styles} label="Last Name" name="lastName"
                    value={formData.lastName} onChange={handleInputChange}
                    placeholder="e.g. Khan" autoComplete="family-name" />
                  <Field styles={styles} label="Contact Number" name="contact"
                    value={formData.contact} onChange={handleInputChange}
                    placeholder="10-digit mobile" autoComplete="tel" inputMode="numeric" />
                  <Field styles={styles} label="Email ID" name="email" type="email" full
                    value={formData.email} onChange={handleInputChange}
                    placeholder="you@company.com" autoComplete="email" />
                </div>

                <div style={styles.actionsEndOnly}>
                  <button type="button" style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => goToStep(2)}>
                    Next →
                  </button>
                </div>
              </>
            ...
            {currentStep === 2 && (
              <>
                <div style={styles.grid}>
                  <Field styles={styles} label="Degree" name="degree"
                    value={formData.degree} onChange={handleInputChange}
                    placeholder="e.g. B.Tech, B.Sc" autoComplete="organization-title" />
                  <Field styles={styles} label="Passout Year" name="passout" type="text"
                    value={formData.passout} onChange={handleInputChange}
                    placeholder="e.g. 2022" inputMode="numeric" autoComplete="off" />
                  <Field styles={styles} label="Experience (years)" name="experience"
                    value={formData.experience} onChange={handleInputChange}
                    placeholder="e.g. 2" inputMode="decimal" autoComplete="off" />
                  <Field styles={styles} label="Tech Stack" name="techstack"
                    value={formData.techstack} onChange={handleInputChange}
                    placeholder="e.g. React, Node, SQL" autoComplete="off" />
                </div>

                <div style={styles.actionsSplit}>
                  <button type="button" style={{ ...styles.btn, ...styles.btnGhost }} onClick={() => goToStep(1)}>
                    ← Previous
                  </button>
                  <button type="button" style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => goToStep(3)}>
                    Next →
                  </button>
                </div>
              </>
            ...
            {currentStep === 3 && (
              <>
                <div style={styles.grid}>
                  <Field styles={styles} label="Last Salary" name="lastSalary"
                    value={formData.lastSalary} onChange={handleInputChange}
                    placeholder="e.g. 6 LPA" autoComplete="off" />
                  <Field styles={styles} label="Expected Salary" name="currentSalary"
                    value={formData.currentSalary} onChange={handleInputChange}
                    placeholder="e.g. 8 LPA" autoComplete="off" />
                  <Field styles={styles} label="Location" name="location"
                    value={formData.location} onChange={handleInputChange}
                    placeholder="e.g. Bengaluru" autoComplete="address-level2" />
                  <Field styles={styles} label="Notice Period" name="noticePeriod"
                    value={formData.noticePeriod} onChange={handleInputChange}
                    placeholder="e.g. 30 days" autoComplete="off" />
                </div>

                <div style={styles.actionsSplit}>
                  <button type="button" style={{ ...styles.btn, ...styles.btnGhost }} onClick={() => goToStep(2)}>
                    ← Previous
                  </button>
                  <button type="button" style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => goToStep(4)}>
                    Next →
                  </button>
                </div>
              </>
            ...
            {currentStep === 4 && (
              <>
                <div style={styles.grid}>
                  <Field styles={styles} label="Skills" name="skills"
                    value={formData.skills} onChange={handleInputChange}
                    placeholder="e.g. React, Redux, REST, Docker" autoComplete="off" />
                  <div style={styles.groupFull}>
                    ...
                    <textarea
                      ...
                      placeholder="Brief about your experience, projects, and impact."
                      style={{ 
                        ...styles.textarea, 
                  ...
                  <div style={styles.groupFull}>
                    ...
                    {formData.cv && !errors.cv && (
                      <span style={{ color: '#0a66c2', fontSize: 13, marginTop: 6 }}>
                ...
                <div style={styles.actionsSplit}>
                  <button type="button" style={{ ...styles.btn, ...styles.btnGhost }} onClick={() => goToStep(3)}>
                    ← Previous
                  </button>
                  <button type="button" style={{ ...styles.btn, ...styles.btnPrimary }} onClick={() => goToStep(5)}>
                    Next →
                  </button>
                </div>
              </>
            ...
            {currentStep === 5 && (
              <>
                <div style={styles.grid}>
                  <Field styles={styles} label="Username" name="username"
                    value={formData.username} onChange={handleInputChange}
                    placeholder="Choose a username" autoComplete="username" />
                  <Field styles={styles} label="Password" name="password" type="password"
                    value={formData.password} onChange={handleInputChange}
                    placeholder="Min 6 characters" autoComplete="new-password" />
                  <Field styles={styles} label="Confirm Password" name="confirmPassword" type="password"
                    value={formData.confirmPassword} onChange={handleInputChange}
                    placeholder="Re-enter password" autoComplete="new-password" />
                </div>

                <div style={styles.actionsSplit}>
                  <button type="button" style={{ ...styles.btn, ...styles.btnGhost }} onClick={() => goToStep(4)}>
                    ← Previous
                  </button>
        ...
        <div style={styles.successBackdrop}>
          <div style={styles.successBox}>
            ...
            <p style={styles.successText}>
              Your profile has been created. You can now apply to roles that fit you best.
            </p>
            <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={handleSuccessClose}>
              Continue to Home
            </button>
          </div>
        </div>
